...
# Generator data generator for input

def get_generator_input_sampler(): #why same size
...
class Generator(nn.Module):
    ...
    # define the sequesnce of the layers with activation fucntions 
    def forward(self, x):
        x = self.map1(x)
        x = self.f(x)
        x = self.map2(x)
...
# Extract the data from the networks
def extract(v):
    return v.data.storage().tolist()

# Show the mean and SD of the data
def stats(d):
...
def train():
    ...
    d_input_size = 500    # Minibatch size
    d_hidden_size = 10    # Hidden layers of discriminator
    d_output_size = 1     # Single dimension for 'real' vs. 'fake' classification
    
    minibatch_size = d_input_size # Size of the minibatch
    ...
    g_steps = 20 # Traning og generator for one epoch
    ...
    generator_activation_function = torch.tanh # Activation function for generator why
    ...
    for epoch in range(num_epochs):
        for d_index in range(d_steps):
            ...
            d_real_error.backward() # Compute and store gradients, but don't change params
            ...
            dre, dfe = extract(d_real_error)[0], extract(d_fake_error)[0] # Store real and fake data errors of discriminator.
